...
const public = {
	show: async function (ownerID) {
		...
		const resident_rows = await db.query(
			`select dorm_name , r_number from resident_student where user_ID = ${ownerID};`);
		const content = JSON.parse(JSON.stringify(rows));
		const resident_content = JSON.parse(JSON.stringify(resident_rows));
